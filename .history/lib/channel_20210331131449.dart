    ...
    router.route("/createUser").linkFunction((request) async {
      ...
      try {
        ...
        accessToken = request.raw.uri.queryParametersAll.entries
            ...
            .value;
        return Response.ok({"key": "$accessToken"});
      }

      final client = HttpClient();
      await client.postUrl(Uri.parse(
          'https://oauth2.googleapis.com/token?code=$responseCode&client_id=${Credentials.clientID}&client_secret=${Credentials.clientSecret}&redirect_uri=http://localhost:8888/createUser&grant_type=authorization_code'));

      return Response.ok({"key": "$requestCode"});
    });
