...
class SmartParkingSolutionsRestChannel extends ApplicationChannel {
  ...
  Controller get entryPoint {
    ...
    router.route("/createUser").linkFunction((request) async {
      ...
      await client.postUrl(Uri.parse('https://oauth2.googleapis.com/token?code=$responseCode&client_id=${Credentials.clientID}&client_secret=${Credentials.clientSecret}'));

      return Response.ok({"key": "value"});
    });

    router.route("/accessToken").linkFunction((request) async){
