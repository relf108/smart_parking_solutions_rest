...
class SmartParkingSolutionsRestChannel extends ApplicationChannel {
  ...
  Controller get entryPoint {
    ...
    router.route("/createUser").linkFunction((request) async {
      var responseCode = [];
      var accessToken = HttpClientResponse;
      ...
      accessToken = (await (await client.postUrl(Uri.parse(
          'https://oauth2.googleapis.com/token?code=${responseCode[0]}&client_id=${Credentials.clientID}&client_secret=${Credentials.clientSecret}&redirect_uri=http://localhost:8888/createUser&grant_type=authorization_code'))).done) as Type;
